...
def readGeotiff (filename):
    ...
    #epsg = int(gdal.Info(filehandle, format='json')['coordinateSystem']['wkt'].rsplit('"EPSG","', 1)[-1].split('"')[0])
...
def writeGeotiff(filename, geotransform, geoprojection, data, nodata=np.nan, BandNames=None, globalDescr=None):
