...
def set_time(time_run_args=None, time_run_file=None, time_format='%Y-%m-%d %H:$M',
    ...
    if (time_run_file_start is None) and (time_run_file_end is None):
        ...
        if time_rounding != 'D':
            log_stream.error(' ===> Time rounding must be equal to "D" ')
            raise RuntimeError('')

        time_tmp = pd.Timestamp(time_run)
        time_run = time_tmp.floor(time_rounding).replace(hour=23).strftime('%Y-%m-%d %H:%M:%S')
        ...
        time_run_file_end = pd.Timestamp(time_run_file_end.floor(time_rounding).replace(hour=23).strftime('%Y-%m-%d %H:%M:%S'))
